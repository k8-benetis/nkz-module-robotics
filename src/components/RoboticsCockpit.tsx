...
const RoboticsCockpit: React.FC = () => {
    ...
    const [latency, setLatency] = useState<number>(45); // Mock latency for phase 1 UI
        ...
        <div className="h-screen w-full bg-slate-950 text-white flex flex-col font-sans overflow-hidden select-none">
            ...
            <header className="h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 shrink-0 z-50">
                <div className="flex items-center space-x-6">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <span className="block font-mono text-sm font-bold text-slate-200 tracking-wider">NKZ-ROBOT-01</span>
                            <span className="block text-[10px] text-slate-500 uppercase tracking-widest">Connected via Zenoh</span>
                        </div>
                    </div>
            ...
            <main className="flex-1 relative bg-black flex items-center justify-center overflow-hidden">
                ...
                <div className="text-slate-600 flex flex-col items-center animate-pulse">
                    ...
                    <p className="text-lg font-light tracking-widest uppercase">Initializing Zenoh Secure Stream</p>
                    <p className="text-xs mt-2 font-mono text-slate-700">Topic: nkz/tenant/robot/cam/front</p>
                </div>
                    ...
                    <>
                        ...
                        <div className="absolute top-6 left-6 p-4 bg-slate-900/40 backdrop-blur-sm border-l-2 border-blue-500 rounded-r-lg">
                            <div className="font-mono text-xs space-y-1 text-blue-100">
                                <div className="flex justify-between w-32"><span>LIN.X:</span> <span className="text-white">0.00 m/s</span></div>
                                <div className="flex justify-between w-32"><span>ANG.Z:</span> <span className="text-white">0.00 r/s</span></div>
                                <div className="flex justify-between w-32"><span>HEAD:</span> <span className="text-white">124° NW</span></div>
                            </div>
                        </div>
                    </>
                )}
            ...
            <footer className="h-52 bg-slate-900 border-t border-slate-800 grid grid-cols-12 divide-x divide-slate-800">
                ...
                <div className="col-span-3 p-5 flex flex-col">
                    <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-4 flex items-center">
                        <Gamepad2 size={12} className="mr-2" /> Traction Control (4WS)
                    </h3>
                    <div className="grid grid-cols-2 gap-3 flex-1">
                        {[
                            { id: 'ACKERMANN_FRONT', label: 'Front', sub: 'Standard', icon: '🚗' },
                            { id: 'ACKERMANN_DUAL', label: 'Dual', sub: 'Sharp Turn', icon: '🚙' },
                            { id: 'CRAB', label: 'Crab', sub: 'Diagonal', icon: '🦀' },
                            { id: 'DIFFERENTIAL', label: 'Pivot', sub: 'Zero Turn', icon: '🚜' },
                        ].map((mode) => (
                            <button
                                key={mode.id}
                ...
                <div className="col-span-6 p-1 relative group">
                    ...
                    <div className="h-full w-full flex items-center justify-center border-2 border-dashed border-slate-800 rounded-lg group-hover:border-slate-700 transition-colors">
                        <div className="text-center">
                            ...
                            <span className="text-xs font-mono text-slate-500">LIDAR / MAP VISUALIZATION REGION</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="col-span-3 p-5 flex flex-col">
                    <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-4">Active Implement</h3>

                    <div className="flex-1 space-y-4">
                        {/* Simulated Widget */}
                        <div className="bg-slate-800/50 p-3 rounded-lg border border-slate-700/50">
                            <div className="flex justify-between items-center mb-2">
                                <span className="text-xs font-bold text-slate-300">Sprayer System</span>
